...
const num2decimals = function(num) {
    let decimals = num.toString().split('').map(x => parseInt(x));
...
class intCodePC {
    ...
    pos2decimals(pos) {
        let decimals = this.data[pos].toString().split('').map(x => parseInt(x));
        let len = decimals.length
        for (let i = 0; i < 5 - len; i ++) {
            decimals.unshift(0)
        }
    ...
    args2nums(args) {
        for (let [i, j] of args.entries()) {
            args[i] = this.modes[i] === 0 ?
                this.readVal(j) :
                (this.modes[i] === 2 ?
                    this.readVal(j + this.relBase) : j);
        }
        return args
    }
