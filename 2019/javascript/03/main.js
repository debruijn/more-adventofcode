...
const findCrossings = function (lph, lpv) {
    ...
    for (let lph_i of lph) {
        for (let lpv_i of lpv) {
            if ((lph_i[0][0] - lpv_i[0][0]) * (lph_i[1][0] - lpv_i[1][0]) <= 0 && (lph_i[0][1] - lpv_i[0][1]) * (lph_i[1][1] - lpv_i[1][1]) <= 0) {
                crossings.push([lph_i[0][1], lpv_i[0][0]])
                steps.push([lph_i[2] + lpv_i[2] + Math.abs(lpv_i[0][0] - lph_i[0][0]) + Math.abs(lph_i[0][1] - lpv_i[0][1])])
            }
        }
    }
