...
const run = function () {
    ...
    let statesPart1 = [0, 1, 2].map(dim => hists[dim].keys().filter(x => hists[dim].get(x) === tPart1[dim]).toArray()[0])

    let potEnergy = [0, 0, 0, 0], kinEWnergy = [0, 0, 0, 0];
    ...
    const resultPart1 = moonsLoc.keys().map(i => potEnergy[i] * kinEWnergy[i]).reduce((a, b) => a + b)
